...
class LinearTaintChecker(object):
    ...
    def get_undeclared_func_type(self):
        return self.linear_types[0]
    ...
    def get_func_declaration(self,func_name,assumptions):
        to_ret = None
        if self.func_type_name in assumptions:
            #print 'Checking decl of:' + func_name + ' in ' + str(assumptions[self.func_type_name])
            if func_name in assumptions[self.func_type_name]:                
                to_ret = assumptions[self.func_type_name][func_name]
        #print 'Returning:' + str(to_ret)
        return to_ret
        ...
        if target_func:
            if len(target_func[args_key]) == len(ast_node.args):
                ...
                for curr_arg in ast_node.args:
                    ...
                    i += 1
                if args_fine:
                    if self.is_linear_type(target_func['ret_type']):
                        #if return type is linear type we need to remove linear type mappings for variables.
                        for curr_arg in ast_node.args:
                            (arg_ret,arg_fine) = self.verify_ast_node(curr_arg,assumptions)
    ...
    def verify_If(self,ast_node,assumptions):
        ...
        if cond_fine:
            if self.is_linear_type(cond_ret):
                self.__error_msg('Error: Linear Type in If condition; Expected non-linear type')
            else:
    ...
    def verify_FuncDef(self,ast_node,assumptions):
        ...
        to_ret = None
        all_fine = False
        #print 'Checking FuncDef:' + str(ast_node)
        if self.has_func_definition(ast_node.decl,assumptions):
            ...
            if self.match_function_declaration(ast_node.decl,assumptions):
                ...
                '''for curr_stat in ast_node.body:
                    #print 'statement:' + str(curr_stat)            
                    (curr_stat_ret,curr_stat_fine) = self.verify_ast_node(curr_stat,new_assumptions)
                    #print 'End statement:' + str(curr_stat)    
                    to_ret = curr_stat_ret
                    all_fine = curr_stat_fine
                    if not curr_stat_fine:
